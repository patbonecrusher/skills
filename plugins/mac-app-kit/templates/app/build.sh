#!/bin/zsh
# Build, bundle, sign and (optionally) notarize or package "__APP_NAME__.app".
#
...
#
# Environment:
#   TEAM_ID              Apple team (default __TEAM_ID__)
...
#   PROVISIONING_PROFILE path to the Mac App Store .provisionprofile (default Resources/__EXEC_NAME__.provisionprofile)
...
TEAM_ID="${TEAM_ID:-__TEAM_ID__}"
BUILD_DIR="$PWD/build"
APP="$BUILD_DIR/$APP_NAME.app"
PROVISIONING_PROFILE="${PROVISIONING_PROFILE:-Resources/__EXEC_NAME__.provisionprofile}"
...
swift build -c "$CONFIG" "${ARCH_FLAGS[@]}" --product __EXEC_NAME__
...
cp "$BIN/__EXEC_NAME__" "$APP/Contents/MacOS/__EXEC_NAME__"
...
ENTITLEMENTS="$BUILD_DIR/__EXEC_NAME__.entitlements"
cp Resources/__EXEC_NAME__.entitlements "$ENTITLEMENTS"
...
if [[ $NOTARIZE == 1 ]]; then
  ...
  ZIP="$BUILD_DIR/__EXEC_NAME__-$VERSION.zip"
  echo "==> Notarizing…"
...
if [[ $PKG == 1 ]]; then
  ...
  PKG_PATH="$BUILD_DIR/__EXEC_NAME__-$VERSION.pkg"
