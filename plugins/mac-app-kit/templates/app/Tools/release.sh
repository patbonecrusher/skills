...
TAG="v$VERSION"
ZIP="build/__EXEC_NAME__-$VERSION.zip"
...
CASK="$TAP_DIR/Casks/__CASK__.rb"
...
git tag -a "$TAG" -m "__APP_NAME__ $VERSION" 2>/dev/null || true
git push origin "$TAG"
gh release create "$TAG" "$ZIP" --title "__APP_NAME__ $VERSION" --generate-notes

echo "==> Updating Homebrew cask"
SHA="$(shasum -a 256 "$ZIP" | cut -d' ' -f1)"
sed -i '' -e "s/version \".*\"/version \"$VERSION\"/" -e "s/sha256 .*/sha256 \"$SHA\"/" "$CASK"
git -C "$TAP_DIR" add Casks/__CASK__.rb
git -C "$TAP_DIR" commit -q -m "Update __REPO__ cask to v$VERSION"
git -C "$TAP_DIR" push -q
echo "==> Done. Install with: brew install --cask __GITHUB_USER__/tap/__CASK__"
